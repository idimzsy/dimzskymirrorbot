    ...
    with download_dict_lock:
        ...
        for index, download in enumerate(list(download_dict.values())[COUNT:], start=1):
            ...
            if STATUS_LIMIT is not None and index == STATUS_LIMIT:
                break
        bmsg = f"<b>🖥𝐂𝐏𝐔:</b> {cpu_percent()}% | <b>💿𝐅𝐫𝐞𝐞:</b> {get_readable_file_size(disk_usage(DOWNLOAD_DIR).free)}"
        bmsg += f"\n<b>🎮𝐑𝐀𝐌:</b> {virtual_memory().percent}% <b>| ⏰𝐔𝐩-𝐓𝐢𝐦𝐞:</b> {get_readable_time(time() - botStartTime)}"
